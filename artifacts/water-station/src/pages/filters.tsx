...
export default function Filters() {
  ...
  const handleToggle = (filter: StationFilter) => {
    updateFilter.mutate({ id: filter.id, data: { isFull: !filter.isFull } }, {
      onSuccess: () => { queryClient.invalidateQueries({ queryKey: getListFiltersQueryKey() }); }
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {filters?.map(filter => (
          <Card key={filter.id} className={`overflow-hidden border-2 transition-colors ${filter.isFull ? 'border-green-400 dark:border-green-600' : 'border-destructive/50'}`}>
            <CardContent className="p-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${filter.isFull ? 'bg-green-100 dark:bg-green-900/40' : 'bg-red-100 dark:bg-red-900/40'}`}>
                ...
                <div className="min-w-0">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${filter.isFull ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
                    {filter.isFull ? "يعمل بشكل جيد" : "يحتاج تغيير"}
                  </span>
                </div>
              </div>
      ...
      {filters && filters.length > 0 && (
        <div className="flex gap-4 text-sm text-muted-foreground">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 inline-block" />{filters.filter(f => f.isFull).length} يعمل</span>
